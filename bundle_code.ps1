...
# Extensions to include (C++, C#, PowerShell, etc.)
$includeExtensions = @(".h", ".cpp", ".hpp", ".bat", ".cs", ".csproj", ".ps1", ".json")
$specificFiles = @("CMakeLists.txt")

# Directories to exclude (relative to root)
$excludeDirs = @("build", "build_sandbox", ".git", ".vs", "BRAIN", "DOCS", "tmp", "out", "bin", "obj", "packages", "dist")
